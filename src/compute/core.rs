...
impl ComputeShader {
    ...
    pub fn from_builder(
        ...
        if !config.storage_buffers.is_empty() {
            ...
            for buffer_spec in &config.storage_buffers {
                if buffer_spec.read_only {
                    resource_layout.add_readonly_storage_buffer(&buffer_spec.name, buffer_spec.size_bytes);
                } else {
        ...
        for i in 0..4 {
            if let Some(layout) = bind_group_layouts.get(&i) {
                ...
                let empty_layout = core.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        ...
        let placeholder_input_texture = if config.has_input_texture {
            Some(Self::create_placeholder_input_texture(&core.device, &format!("{} Placeholder Input", config.label)))
        } else {
            None
        };
        ...
        let mut empty_bind_groups: std::collections::HashMap<u32, wgpu::BindGroup> = std::collections::HashMap::new();
        for i in 0..4 {
        ...
        let (multipass_manager, pass_dependencies) = if let Some(passes) = &config.passes {
            if config.storage_buffers.is_empty() {
                ...
                let dependencies: HashMap<String, Vec<String>> = passes.iter()
    ...
    fn create_placeholder_input_texture(
        device: &wgpu::Device,
        label: &str,
    ) -> TextureManager {
    ...
    fn create_engine_resources(
        ...
        let mouse_uniform = if config.has_mouse {
            ...
            let temp_layout = core.device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
    ...
    pub fn dispatch_stage_with_workgroups(&mut self, encoder: &mut wgpu::CommandEncoder, stage_index: usize, workgroup_count: [u32; 3]) {
        
        if stage_index >= self.pipelines.len() {
            log::error!("Stage index {} out of bounds (max: {})", stage_index, self.pipelines.len() - 1);
            return;
        }
        ...
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some(&format!("{} Stage {} - {}", self.label, stage_index, entry_point)),
    ...
    pub fn dispatch_stage(&mut self, encoder: &mut wgpu::CommandEncoder, core: &Core, stage_index: usize) {
        let workgroup_count = [
    ...
    pub fn update_input_texture(&mut self, texture_view: &wgpu::TextureView, sampler: &wgpu::Sampler, device: &wgpu::Device) {
    ...
    pub fn update_channel_texture(&mut self, channel_index: u32, texture_view: &wgpu::TextureView, sampler: &wgpu::Sampler, device: &wgpu::Device, queue: &wgpu::Queue) {
        
        if channel_index >= self.num_channels {
            log::warn!("Attempted to update channel {} but only {} channels are configured", channel_index, self.num_channels);
            return;
        }
        
        // Store the channel texture
        self.channel_textures.insert(channel_index, Some((texture_view.clone(), sampler.clone())));
    ...
    fn initialize_channel_textures(num_channels: u32) -> HashMap<u32, Option<(wgpu::TextureView, wgpu::Sampler)>> {
    ...
    fn dispatch_single_pass(&self, encoder: &mut wgpu::CommandEncoder, _core: &Core, workgroup_count: [u32; 3]) {
    ...
    fn dispatch_multipass(&mut self, encoder: &mut wgpu::CommandEncoder, core: &Core, workgroup_count: [u32; 3]) {
        ...
        for pass_idx in 0..num_passes {
            ...
            let input_bind_group = if let (Some(multipass), Some(dependencies)) = (&self.multipass_manager, &self.pass_dependencies) {
            ...
            if entry_point == "main_image" {
                ...
                if let Some(multipass) = &self.multipass_manager {
                    ...
                    let write_view = write_texture.create_view(&wgpu::TextureViewDescriptor::default());
                    ...
                    let group1_layout = self.bind_group_layouts.get(&1).unwrap();
                    let intermediate_bind_group = core.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            if let Some(ref group2) = self.group2_bind_group {
                ...
                log::warn!("Using empty Group 2 bind group for pass {} - channels won't work!", entry_point);
                compute_pass.set_bind_group(2, empty_group2, &[]);
            } else {
                log::error!("No Group 2 bind group available - this shouldn't happen with contiguous layout");
            }
    ...
    pub fn enable_hot_reload(&mut self, device: Arc<wgpu::Device>, shader_path: PathBuf, shader_module: wgpu::ShaderModule) -> Result<(), notify::Error> {
            ...
            if let Some(new_module) = hot_reload.reload_compute_shader() {
                ...
                for entry_point in &self.entry_points {
                    let new_pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
                        label: Some(&format!("Updated {} Pipeline - {}", self.label, entry_point)),
                ...
                info!("{} shader hot-reloaded at frame: {}", self.label, self.current_frame);
                return true;
            }
        }
        false
    ...
    pub fn update_mouse_uniform(&mut self, mouse_uniform_data: &crate::MouseUniform, queue: &wgpu::Queue) {

...
impl ShaderManager for Shader {
    fn init(core: &Core) -> Self {
        ...
        let mut compute_shader = ComputeShader::from_builder(
            core,
            include_str!("shaders/cuneus.wgsl"),
            config,
        );
                    ...
                    .show(ctx, |ui| {
                        egui::CollapsingHeader::new("Colors")
                            ...
                            .show(ui, |ui| {
                                ...
                                changed |= ui.color_edit_button_rgb(&mut params.hue_color).changed();
                                ui.label("Base Color");
                            });
                        ...
                        egui::CollapsingHeader::new("Lighting")
                            .default_open(false)
                            .show(ui, |ui| {
                                changed |= ui.add(egui::Slider::new(&mut params.light_intensity, 0.0..=3.2)
                                    .text("Light Intensity")).changed();
                                    
                                changed |= ui.add(egui::Slider::new(&mut params.ao_strength, 0.0..=10.0)
                                    .text("AO Strength")).changed();
                                    
                                changed |= ui.add(egui::Slider::new(&mut params.global_light, 0.1..=2.0)
                                    .text("Global Light")).changed();
                                    
                                changed |= ui.add(egui::Slider::new(&mut params.rim_power, 0.1..=10.0)
                                    .text("Rim Power")).changed();
                                    
                                changed |= ui.add(egui::Slider::new(&mut params.env_light_strength, 0.0..=1.0)
                                    .text("Environment Light")).changed();
                            });
                
                        egui::CollapsingHeader::new("Effects")
                            .default_open(false)
                            .show(ui, |ui| {
                                changed |= ui.add(egui::Slider::new(&mut params.alpha_threshold, 0.0..=3.0)
                                    .text("Alpha Threshold")).changed();
                                    
                                changed |= ui.add(egui::Slider::new(&mut params.mix_factor_scale, 0.0..=1.5)
                                    .text("Mix Factor Scale")).changed();
                                    
                                changed |= ui.add(egui::Slider::new(&mut params.iridescence_power, 0.0..=1.0)
                                    .text("Iridescence")).changed();
                                    
                                changed |= ui.add(egui::Slider::new(&mut params.falloff_distance, 0.5..=5.0)
                                    .text("Light Falloff")).changed();
                            });
